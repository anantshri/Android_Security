...
#include "libt.h"
#include "util.h"
#include "phDal4Nfc_i2c_read.h"

int (*orig_phDal4Nfc_i2c_read)(char * pBuffer, int nNbBytesToRead);

/*
void my_cb_phDal4Nfc_i2c_read(...) // FIXME
{
...
//	log("%s enter\n", __func__)
//	log("orig_phDal4Nfc_i2c_read = %x\n", orig_phDal4Nfc_i2c_read)
	int i;
	struct special_phDal4Nfc_i2c_read_t *d = (struct special_phDal4Nfc_i2c_read_t*)hook_phDal4Nfc_i2c_read.data;

	hook_precall(&hook_phDal4Nfc_i2c_read);
	int res = orig_phDal4Nfc_i2c_read(pBuffer, nNbBytesToRead);
