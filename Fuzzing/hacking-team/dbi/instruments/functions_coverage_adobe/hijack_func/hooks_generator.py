...
for i in functions_ra:
    ...
    if(  int(i.strip(),16)-1 in blacklist): continue # int(i.strip(),16) < 0x11bb59 or int(i.strip(),16) > 0x45597e or
    out = "#define HOOK_coverage_%d hook(hook_hash1, pid, \"libAdobeReader\", \"\", no_proto, 0, %s );\n" % (j, i.strip())
    header.write(out)
    j+=1
