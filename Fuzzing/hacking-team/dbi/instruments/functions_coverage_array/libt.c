...
static int dispatcher_array_next_position = 0;
...
int hook(struct hook_t *hook_hash, int pid, char *libname, char *funcname, void *hookf, int by_name, unsigned int raw_address)
{
	...
	if( DEBUG)
	  log("[*]Adding 0x%x to hash table 0x%x\n", hook->orig, hook_hash)
	    ...
	    ;
	log("[*] Adding 0x%x in position %d\n", addr, dispatcher_array_next_position )
	//HASH_ADD_INT(hook_hash1, orig, hook);
	struct dispatcher_t *d  = (struct dispatcher_t *) malloc(sizeof(struct dispatcher_t));
	d->address = addr;
	d->hook = hook;


	dispatcher_array[dispatcher_array_next_position] =  d;

	dispatcher_array_next_position +=1;
