...
blacklist = [0x2f5311, 0x48bc35, 0x484e11]
...
header.write("struct hook_t *postcall_hash;\n\n")
...
for i in functions_ra:
    ...
    out = "#define HOOK_coverage_%d hook(hook_hash1, pid, \"libpolarisoffice4\", \"\", no_proto, 0, %s );\n" % (j, i.strip())
    header.write(out)
    j+=1
