...
void* (*access_arm_ptr)(void* a, void* b);
...
void*  access_arm(void* a, void* b)  {
  ...
  access_arm_ptr = (void *) hook_no_proto->orig;
  hook_precall(hook_no_proto);

  result = access_arm_ptr(a, b);

  //log("%x arm %x\n", ip_var - hook_no_proto->lib_base_addr, hook_no_proto->lib_base_addr) // not needed here

  if(strcmp(filename, (void *)r0) && (strstr((void *)r0, ".jpg") != NULL)) {
    log("file:%s \n", r0)
  ...
  // Install again the hook for libc access
